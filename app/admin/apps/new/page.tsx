...
export default function AddNewApp() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Generate slug
      const slug = await createSlug(formData.title, "apps")
      ...
      if (useIconUrl) {
        ...
        const result = await uploadFile(appIcon, "app-icons", slug)
        if (!result.success) throw new Error("Failed to upload app icon")
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-800 dark:text-white">Add New App</h1>
            <p className="text-gray-500 dark:text-gray-400">Create a new app entry</p>
          </div>
        </div>
