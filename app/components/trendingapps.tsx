...
import { ChevronRight, Zap, Download, TrendingUp } from "lucide-react"
  ...
  useEffect(() => {
    const fetchTrendingApps = async () => {
      try {
        ...
        // Fetch trending apps from the trending_apps table
        ...
        if (trendingData && trendingData.length > 0) {
          ...
          const sortedApps = contentIds.map((id) => trendingApps.find((app) => app.id === id)).filter(Boolean)
      ...
      <section className="mb-10">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-gradient-to-br from-pink-400 to-pink-600 rounded-full flex items-center justify-center shadow-lg relative">
              <TrendingUp className="w-5 h-5 text-white" />
              <div className="absolute inset-0 bg-pink-400 rounded-full blur-md opacity-30 -z-10"></div>
            </div>
            <h2 className="text-xl font-bold text-gray-800 dark:text-white">Trending Apps</h2>
          </div>
          <Link
            href="/trendingapps"
    ...
    return (
      <section className="mb-10">
        ...
        <div className="bg-red-50 dark:bg-red-900/20 p-4 rounded-xl text-red-600 dark:text-red-400">
          Error loading trending apps: {error}
        </div>
      </section>
    )
  }
