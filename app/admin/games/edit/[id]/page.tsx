...
export default function EditGame({ params }: { params: { id: string } }) {
  ...
  // Add favicon state variables after the existing image states
  ...
  // Fetch game data and categories on component mount
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        setFormData({
          ...
          size: gameData.size || "",
          mod_info: gameData.mod_info || "",
          description: gameData.description || "",
          google_play_link: gameData.google_play_link || "",
          ...
          // Open Graph fields
          og_title: gameData.og_title || "",
          og_description: gameData.og_description || "",
          og_type: gameData.og_type || "article",
          og_url: gameData.og_url || "",
          // Twitter Card fields
          twitter_title: gameData.twitter_title || "",
          twitter_description: gameData.twitter_description || "",
          twitter_card: gameData.twitter_card || "summary_large_image",
          icon_bg_color: gameData.icon_bg_color || "#3E3E3E",
        })
        ...
        if (gameData.icon_url) {
          setExistingIconPath(gameData.icon_url)
          setGameIconPreview(getPublicUrl(gameData.icon_url, "app-icons"))
        }

        // Set existing OG image
        if (gameData.og_image) {
          setExistingOgImagePath(gameData.og_image)
          setOgImagePreview(getPublicUrl(gameData.og_image, "seo-images"))
        }

        // Set existing Twitter image
        if (gameData.twitter_image) {
          setExistingTwitterImagePath(gameData.twitter_image)
          setTwitterImagePreview(getPublicUrl(gameData.twitter_image, "seo-images"))
        }

        // Add this to the useEffect that fetches game data
        // After setting existing icon, OG image, Twitter image, etc.

        // Set existing favicon
        if (gameData.favicon_url) {
          setExistingFaviconPath(gameData.favicon_url)
          setFaviconPreview(getPublicUrl(gameData.favicon_url, "app-icons"))
        }

        // Set existing APK
        if (gameData.download_url) {
          setExistingApkPath(gameData.download_url)
        }
  ...
  // Add favicon handlers after the existing image handlers
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (replaceIcon) {
        if (useIconUrl) {
          ...
          if (bucketStatus.existingBuckets.includes("app-icons")) {
            ...
            if (!result.success) {
              throw new Error(result.error?.message || "Failed to upload game icon")
            }

            iconUrl = result.path // Store the path, not the full URL
          } else {
            throw new Error(
              "Cannot upload game icon: Storage bucket 'app-icons' does not exist. Please use a direct URL instead.",
            )
          }
        }
      }
      ...
      await supabase.from("mod_features").delete().eq("content_id", id).eq("content_type", "game")
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-800 dark:text-white">Edit Game</h1>
            <p className="text-gray-500 dark:text-gray-400">Update game details and media</p>
          </div>
        </div>
          ...
          {activeTab === "basic" && (
            <div className="grid grid-cols-1 gap-6">
              <div>
                ...
                />
                <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
                  If left empty, the game title and version will be used.
                </p>
              </div>
              ...
              <div>
                ...
                />
                <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
                  Example: game name, mod apk, premium, unlocked
                </p>
              </div>
            </div>
          )}
          ...
          {activeTab === "og" && (
            <div className="grid grid-cols-1 gap-6">
              <div>
                ...
                />
                <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
                  If left empty, the meta title or game title will be used.
                </p>
              </div>
              ...
              <div>
                ...
                />
                <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
                  If left empty, the game's URL will be used.
                </p>
              </div>
          ...
          {activeTab === "twitter" && (
            <div className="grid grid-cols-1 gap-6">
              ...
              <div>
                ...
                />
                <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
                  If left empty, the OG title or game title will be used.
                </p>
              </div>
                ...
                {replaceIcon && (
                  <button
                    ...
                    onClick={() => {
                      ...
                      setGameIcon(null)
                      setGameIconUrl("")
                      setGameIconPreview(existingIconPath ? getPublicUrl(existingIconPath, "app-icons") : null)
                    }}
                    className="px-3 py-1 text-xs rounded-full bg-red-500 text-white"
                  >
                    Keep Existing
